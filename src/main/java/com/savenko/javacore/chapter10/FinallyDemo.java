...
public class FinallyDemo {
    //сгенеррировать исключение в методе
    static void procA(){
        try {
            System.out.println("В теле метода procA()");
            throw new RuntimeException("демострация");
        } finally {
            System.out.println("Блок оператора finally в методе procA()");
        }
    }
    //возварить управление из блока оператора try
    static void procB(){
        try {
    ...
    static void procC(){
        try {
            ...
            System.out.println("Юлок оператора finally в методе procC()");
        }
    }
